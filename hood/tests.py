...
class ProfileTest(TestCase):
    def setUp(self):
        ...
        self.user.save()
        self.neighbourhood =  Neighbourhood(hood_name = "Hood", hood_location= "Nairobi", admin = self.user,hood_description='area', hood_photo="hood.jpg")
        self.neighbourhood.save()
        self.profile = Profile(id=4,idNo=39607876,email='email@g.com', profile_pic='profile.jpg', bio='this is a test profile',neighbourhood=self.neighbourhood,
                                    user=self.user)
...
class BusinessTest(TestCase): 
    ...
    def setUp(self):
        ...
        self.neighbourhood =  Neighbourhood(hood_name = "Hood", hood_location= "Nairobi", admin = self.user,hood_description='area', hood_photo="hood.jpg")
        self.neighbourhood.save()
        self.biz = Business(user=self.user,business_name="saloon", business_hood=self.neighbourhood,business_email="saloon@gmail.com", business_desc="plaiting")
        self.biz.save()
...
class PostTest(TestCase): 
    ...
    def setUp(self):
        ...
        self.neighbourhood =  Neighbourhood(hood_name = "Hood", hood_location= "Nairobi", admin = self.user,hood_description='area', hood_photo="hood.jpg")
        self.neighbourhood.save()
        self.post = Post(user=self.user,title="saloon",image="post.jpg" ,content ="my post", timestamp=datetime.datetime,neighbourhood=self.neighbourhood)
        self.post.save()
